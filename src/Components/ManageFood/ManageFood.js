...
const ManageFood = () => {
  ...
  const updateData = () => {
    ...
    fetch(`https://restaurant-server5.onrender.com/update/${updateID}`, {
      method: "PATCH",
  ...
  const deleteFood = (id) => {
    fetch(`https://restaurant-server5.onrender.com/delete/${id}`, {
      method: "DELETE",
    });
  };
          ...
          {manageFood.map((food) => (
            <tbody>
              <tr>
                ...
                <td>{food.price}</td>
                <td>
                  <Button onClick={() => deleteFood(food._id)} variant="danger">
                    Delete
                  </Button>
                </td>
                <td>
                  <Button onClick={() => editFood(food._id)} variant="success">
                    Update
                  </Button>
                </td>
              </tr>
