...
const StaffList = () => {
    ...
    const updateStaff = () => {
        ...
        const updateData = {updateName, updateAddress, updateNumber, updateSalary};

        fetch(`http://localhost:5000/updateStaff/${updateID}`, {
            method: 'PATCH',
        ...
        <div>
            ...
            <Container>
                <Table striped bordered hover>
                    ...
                    {
                        staff.map(staffs => <tbody><tr><td>{staffs.name}</td><td>{staffs.address}</td><td>{staffs.number}</td><td>{staffs.salary}</td><td><Button onClick={() => deleteStaff(staffs._id)} variant="danger">Delete</Button></td><td><Button onClick={() => editStaff(staffs._id)} variant="success">Edit</Button></td></tr></tbody>)
                    }
                </Table>
                ...
                <div className="from-group pt-3" >
                    <input type="number" id='salary' className="form-control" name='salary' required />
                </div>
